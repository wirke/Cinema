...
class Rezervacija {
  static async sveRezervacije() {
    return db.any('SELECT r.*, p.vremePrikaza, f.naziv AS naziv_filma, pr.naziv AS naziv_prostorije FROM rezervacija r JOIN prikaz p ON r.id_prikaz = p.id JOIN film f ON p.id_film = f.id JOIN prostorija pr ON p.id_prostorija = pr.id');
  }
